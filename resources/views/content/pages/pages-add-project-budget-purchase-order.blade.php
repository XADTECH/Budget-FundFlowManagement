...
@section('content')
                            ...
                            <div class="col-sm-4">
                                ...
                                <select class="form-select" name="supplier_name">
                                    ...
                                    @foreach ($supplierlist as $supplier)
                                        <option value="{{ $supplier->supplier_name }}">{{ $supplier->supplier_name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="row mt-4">
                            <div class="col-sm-4">
                                ...
                                <input type="text" class="form-control" name="supplier_address"
                                    placeholder="e.g., Abu Hail Dubai, UAE" />
                            </div>

                            <!-- Dropdown Container for Searching Project -->
                            <div class="col-sm-4 dropdown-container">
                                <label for="project_search" class="form-label">Choose Project</label>
                                <!-- Text input to filter project list -->
                                <input type="text" id="project_search" class="form-control dropdown-search"
                                    placeholder="Search for project...">
                                <!-- Hidden input to hold the selected project_id -->
                                <input type="hidden" id="project_id_hidden" name="project_id" />

                                <!-- Actual dropdown for projects (no name attribute) -->
                ...
                <tbody id="project-table-body" class="table-border-bottom-0">
                    @if ($purchaseOrders->isEmpty())
                        <tr>
                            ...
                            <!-- Adjust colspan for new Action column -->
                        </tr>
                    @else
                        @foreach ($purchaseOrders as $po)
                            ...
                            <tr>
                                ...
                                <!-- Prepared By -->
                                <td>
                                    {{ optional($preparedPerson)->first_name . ' ' . optional($preparedPerson)->last_name ?? 'N/A' }}
                                </td>

                                <!-- Requested By -->
                                <td>
                                    {{ optional($requestPerson)->first_name ?? 'N/A' }}
                                </td>
                                ...
                                <!-- Action -->
                                <td>
                                    <form action="{{ route('purchaseOrder.destroy', ['POID' => $po->po_number]) }}"
                                        method="POST"
    ...
    <!-- Modal for Editing Project (if needed) -->

                ...
                <form id="filterForm" action="{{ route('filter-purchase-orders') }}" method="GET">
                    <div class="row">
                        <div class="col-sm-4">
                            ...
                            <select class="form-select" name="project_id" id="project_id">
                                ...
                                @foreach ($projects as $project)
                                    <option value="{{$project->id}}" {{ request('project_id') == $project->id ? 'selected' : '' }}>{{$project->reference_code}}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-sm-4">
                            ...
                            <select class="form-select" name="project_person_id" id="project_person_id">
                                ...
                                @foreach ($users as $user)
                                    <option value="{{$user->id}}" {{ request('project_person_id') == $user->id ? 'selected' : '' }}>{{$user->first_name}}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-sm-4">
                            ...
                            <input type="date" id="date" class="form-control" name="date" value="{{ request('date') }}" placeholder="Enter Date" />
                        </div>
                    </div>
                    ...
                    <div class="row mt-4">
                        <div class="col-sm-4">
                            ...
                            <input type="text" id="po_number" class="form-control" name="po_number" value="{{ request('po_number') }}" placeholder="Enter PO Number" />
                        </div>
                    </div>
                    ...
                    <tbody id="project-table-body" class="table-border-bottom-0">
                        @foreach($purchaseOrders as $po)
                            ...
                            <tr>
                                ...
                                <td> <span style="color: red;font-weight:700">{{ $po->is_verified ? 'Verified' : 'Not Verified' }}</span></td>

...
@section('content')
    ...
    // Check if the purchase order is submitted or not
    if (purchaseOrderStatus === 'submitted') {
        try {
            ...
            itemsToRender = data.items || []; // Use items from the server response

            // Update discount, VAT, and delivery charges from the server response
            ...
            document.getElementById('totalDiscount').textContent = parseFloat(data.total_discount).toFixed(2);
            document.getElementById('totalVAT').textContent = parseFloat(data.total_vat).toFixed(2);
        ...
        // Use locally available purchase order items
    ...
    // Call updateTotals to refresh calculations
    updateTotals();
}
    ...
    `);
                ...
                function submitData() {
                    ...
                    const discountValue = parseFloat(document.getElementById('discountInput').value) || 0;
                    const vatValue = parseFloat(document.getElementById('vatInput').value) || 0;
                    const initialBudget = parseFloat(document.getElementById('budgetDisplay').textContent.replace(/,/g, '')) || 0;
                    const utilization = parseFloat(document.getElementById('utilize').textContent.replace(/,/g, '')) || 0;
                    const budgetBalance =  parseFloat(document.getElementById('balance_budget').textContent.replace(/,/g, '')) || 0;
                    const requestAmount =  parseFloat(document.getElementById('total_request_amount').textContent.replace(/,/g, '')) || 0;

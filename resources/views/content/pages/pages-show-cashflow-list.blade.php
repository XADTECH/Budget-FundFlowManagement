    ...
    <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex">
            <form class="d-flex" method="GET" action="{{ route('budgets.cashflowLists') }}">
                <input type="text" name="reference_code" class="form-control me-2" placeholder="Budget Reference Code" aria-label="Search" value="{{ request('reference_code') }}">
            ...
            <div class="col-md-4">
                ...
                <select class="form-select" name="budget_project_id">
                    ...
                    @foreach ($budgetProjects as $budgetProject)
                        <option {{ request('budget_project_id') == $budgetProject->id ? 'selected' : '' }} value="{{ $budgetProject->id }}">{{ $budgetProject->reference_code }}</option>
                    @endforeach
                </select>
            </div>
        </div>

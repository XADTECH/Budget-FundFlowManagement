...
class PdfController extends Controller
{
    public function download($POID)
    {
        ...
        $purchaseOrder = PurchaseOrder::where('po_number', $POID)->firstOrFail();
        $purchaseOrderItem = PurchaseOrderItem::where('purchase_order_id', $purchaseOrder->id)->firstOrFail();
        
        // Decode items JSON
        $purchaseOrderItems = json_decode($purchaseOrderItem->items, true);
        
        // Fetch related budget and project details
        $budget = BudgetProject::where('id', $purchaseOrderItem->project_id)->first();
        ...
        $pdf = PDF::loadView('content.pages.pdf.pages-budget-project-summary-report', [
        ...
        return $pdf->stream("PurchaseOrder_{$POID}.pdf");
    }
